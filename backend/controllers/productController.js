...
const getProductsBySubcategory = async (req, res) => {
  ...
  try {
    ...
    const productsData = await ProductModel.find({ category: id1, subcategory: id2 });
...
const updateProduct = async (req, res) => {
  try {
    ...
    const productData = await ProductModel.findByIdAndUpdate({ _id : id}, {...req.body});

    if (!productData) {
      return res.status(404).json({ error: 'Such product not found' });
    }

    res.status(200).json({ message: `Product id: ${id} is updated`, productData});
  } catch (error) {
    res.status(500).json({ message: 'Internal server error' });
  }
};
...
const deleteProduct = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Product id: ${id} is deleted`, productData});
  } catch (error) {
    res.status(500).json({ message: 'Internal server error' });
  }
};
...
const deleteCategory = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Category id: ${id} is deleted`, productsData});
  } catch (error) {
    res.status(500).json({ message: 'Internal server error' });
  }
};
...
const deleteSubcategory = async (req, res) => {
  try {
    const { id1, id2 } = req.params;
    const productsData = await ProductModel.deleteMany({ category: id1, subcategory: id2 });

    if (!productsData) {
      return res.status(404).json({ message: 'Such category not found' });
    }

    res.status(200).json({ message: `Subcategory: ${id2} is deleted from category: ${id1}`, productsData});
  } catch (error) {
    res.status(500).json({ message: 'Internal server error' });
  }
};
